...
def constructBayesNet(gameState: hunters.GameState):
    ...
    legal_pos = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    variableDomainsDict[PAC] = legal_pos
    variableDomainsDict[GHOST0] = legal_pos
    variableDomainsDict[GHOST1] = legal_pos
    values = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))
    variableDomainsDict[OBS0] = values
    variableDomainsDict[OBS1] = values
    "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            self.beliefs[pos] *= self.getObservationProb(observation, cur_pos, pos, jail_pos)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        getDist = {pos:self.getPositionDistribution(gameState, pos) for pos in self.allPositions}
        # For each possible new position
        for newpos in self.allPositions:
            for oldpos in self.allPositions:
                if self.beliefs[oldpos] > 0:
                    newBeliefs[newpos] += getDist[oldpos][newpos] * self.beliefs[oldpos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        num_pos = len(legal_pos)
        for i in range(num):
            x, y = legal_pos[i % num_pos]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            prob = self.getObservationProb(observation, gameState.getPacmanPosition(), particle,
        ...
        if weight.total() == 0:
            ...
            for i in range(self.numParticles):
                newParticles.append(weight.sample())
    ...
    def elapseTime(self, gameState):
        ...
        newBeliefs = DiscreteDistribution()
        for pos, value in beliefs.items():
            newPosDist = self.getPositionDistribution(gameState, pos)
            for newPos, newValue in newPosDist.items():
                newBeliefs[newPos] += value * newValue
        self.particles.clear()
        for pos, value in newBeliefs.items():
            import math
            count = math.floor(value * self.numParticles)
            self.particles.extend([pos] * count)
        "*** END YOUR CODE HERE ***"
